...
const failedRequests = require('./failure.js');
...
module.exports = function getMetadata(font) {
  ...
  font.dataFiles.forEach((data) => {
    ...
    /* If the file is a TTF font file, cache it name and URL */
  ...
  function parseMetadata(font, content) {
    ...
    if (font.metadata.name) {
      ...
      font.webfontLink = fontURL + '?' + query;
